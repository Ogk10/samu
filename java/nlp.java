...
class NLP
{
  ...
  public List<STriplet> sentence2triplets ( String sentence ) 
  {
  
	List<STriplet> triplets = new ArrayList<STriplet>();
	
	LinkGrammar.parse(sentence);
	
	int num_linkages = LinkGrammar.getNumLinkages();

	STriplet triplet = new STriplet();
	...
	{   
		...
		if ( c.charAt(0) == 'S' )
		{
			...
			alt = words[LinkGrammar.getLinkRWord(k)];
		...
		if ( c.charAt(0) == 'O' )
		{
		    ...
		    else if ( alt.equals(words[LinkGrammar.getLinkRWord(k)]) )
		    {
			triplet.p = alt;
			triplet.cut ( );
