...
public QL() 
{
	...
	prcps = new HashMap<STriplet, Perceptron>();
	frqs = new HashMap<STriplet, Map<String, Integer>>();
}
...
double max_ap_Q_sp_ap(double image[]) 
{
	...
	for (Iterator<Entry<STriplet, Perceptron>> it = prcps.entrySet().iterator(); it.hasNext();) 
	{
		Entry<STriplet, Perceptron> thisEntry = (Entry<STriplet, Perceptron>) it.next();
...
STriplet argmax_ap_f(String prg, double image[]) 
{
	...
	for (Iterator<Entry<STriplet, Perceptron>> it = prcps.entrySet().iterator(); it.hasNext();) 
	{
		...
		{
			min_f = explor;
			ap = (STriplet) thisEntry.getKey();
 		}
	}
	return ap;
}

STriplet ThisWasAnOperator(STriplet triplet, String prg, double image[]) 
{
...
private Map<STriplet, Perceptron> prcps;

private Map<STriplet, Map<String, Integer>> frqs;

private STriplet prev_action;
