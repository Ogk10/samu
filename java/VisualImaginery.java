...
class VisualImagery
{
	...
	{
		app = new LinkedList<STriplet>();
		ql = new QL();
  	}

	public void putIn(List<STriplet> triplets) {

    	if ( triplets.size() ==0)
      		return;

	for ( STriplet triplet : triplets )
        {
        	if ( app.size() >= stmt_max )
        	  app.poll();
